...
export class UtController {
    ...
    async convertUtmToGeo(request: Request, response: Response) : Promise<Response> {
        const { latitude, longitude } = await utService.utmToGeo(request.body)
    ...
    async convertGeoToUtm(request: Request, response: Response) : Promise<Response> {
        const { este, norte } = await utService.geoToUtm(request.body)
        return response.status(200).json({
            este,
            norte
        })
    ...
    async getGeoJsonShape(request: Request, response: Response) {
        const { id, type }: any = request.query

        const result = await utService.getGeoJsonShape(request.user?.id, type, id)
