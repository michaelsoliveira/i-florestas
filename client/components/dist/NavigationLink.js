"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __rest = (this && this.__rest) || function (s, e) {
    var t = {};
    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
        t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) {
            if (e.indexOf(p[i]) < 0 && Object.prototype.propertyIsEnumerable.call(s, p[i]))
                t[p[i]] = s[p[i]];
        }
    return t;
};
exports.__esModule = true;
exports.NavigationLink = void 0;
var router_1 = require("next/router");
var prop_types_1 = require("prop-types");
var Link_1 = require("./Link");
NavigationLink.propTypes = {
    href: prop_types_1["default"].string.isRequired,
    exact: prop_types_1["default"].bool
};
NavigationLink.defaultProps = {
    exact: false
};
function NavigationLink(_a) {
    var children = _a.children, href = _a.href, exact = _a.exact, props = __rest(_a, ["children", "href", "exact"]);
    var pathname = router_1.useRouter().pathname;
    var isActive = exact ? pathname === href : pathname.startsWith(href);
    if (isActive) {
        props.className += ' active';
    }
    return React.createElement(Link_1.Link, __assign({ href: href }, props), children);
}
exports.NavigationLink = NavigationLink;
