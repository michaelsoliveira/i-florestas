...
import { decimalParaGms } from "../utils/ConvertCoords"
...
const Index = ({ currentUts, onPageChanged, changeItemsPerPage, orderBy, order, currentPage, perPage, loading, loadUts }: any) => {
    ...
    const deleteSingleModal = (id?: string) => showModal({ title: 'Deletar UT', onConfirm: () => { deleteUt(id) }, styleButton: styles.redButton, iconType: 'warn', confirmBtn: 'Deletar', content: <>Tem Certeza que deseja excluir a UT {utById(id)?.numero_ut} ? <br /> Esta ação também excluirá todas as árvores relacionadas a esta UT.</> })
    const deleteMultModal = () => showModal({ type: 'delete.ut', title: 'Deletar UTs', onConfirm: deleteUts, styleButton: styles.redButton, iconType: 'warn', confirmBtn: 'Deletar', content: <>Tem certeza que deseja excluir as UTs selecionadas? <br /> Esta ação também excluirá todas as árvores relacionadas as UTs selecionadas.</> })
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        ...
                        <tr key={ut.id}>
                            ...
                            </td>
                            {
                                upa?.tipo === 1 && (
                                    <td className="px-3 py-2 whitespace-nowrap">
                                        <div className="text-sm text-gray-900">Latitude:&nbsp;{ decimalParaGms(ut?.latitude) }&nbsp;Longitude: { decimalParaGms(ut?.longitude) }</div>
                                    </td>
                                )
                            }
                            <td className="px-3 py-2 whitespace-nowrap">
