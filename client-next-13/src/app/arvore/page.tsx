...
const ArvoreIndex = () => {
    ...
    const loadArvores = useCallback(async (itemsPerPage?: number, currentPage?: number) => {
        setLoading(true)
        const currentPagePagination = (pagination.name === usePathname() && pagination.currentPage) ? pagination.currentPage : 1
    ...
    }, [client, order, orderBy, pagination.currentPage, pagination.name, pagination.perPage, usePathname, setLoading, ut?.id])
