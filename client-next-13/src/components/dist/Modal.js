...
function Modal(props) {
    ...
    var styleButton = store.styleButton, className = store.className, title = store.title, size = store.size, visible = store.visible, confirmBtn = store.confirmBtn, onConfirm = store.onConfirm, iconType = store.iconType, content = store.content, type = store.type, hookForm = store.hookForm, _b = store.options, options = _b === void 0 ? true : _b, cancelName = store.cancelName;
                    ...
                    React.createElement("div", { className: classNames_1["default"]('w-full inline-block align-bottom bg-white rounded-lg text-left shadow-xl transform transition-all sm:my-8 sm:align-middle', size ? size : 'sm:max-w-md') },
                        React.createElement("div", null,
                            React.createElement("div", { className: "bg-white px-4 rounded-full" },
                                React.createElement("div", { className: "sm:flex sm:items-center py-2 space-x-2" },
                                    (iconType === 'warn') && (React.createElement("div", { className: "mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10" },
                                        React.createElement(outline_1.ExclamationTriangleIcon, { className: "h-6 w-6 text-red-600", "aria-hidden": "true" }))),
                            ...
                            options && (React.createElement("div", { className: "bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse rounded-b-full" },
                                confirmBtn && (React.createElement("button", { type: type === "submit" ? 'submit' : 'button', className: classNames_1["default"]('w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm', styleButton), onClick: onConfirm, form: hookForm ? hookForm : '', ref: submitButtonRef }, confirmBtn)),
                                React.createElement("button", { ref: cancelButtonRef, type: "button", className: "mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm", onClick: hideModal }, cancelName ? cancelName : 'Cancelar'))))))))));
}
